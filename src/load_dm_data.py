...
DM_TABLES = {
    "dict_currency": "data/raw/dict_currency/dict_currency.csv"
}
...
def load_to_stage(table_name):
    try:
        df = pd.read_csv(DM_TABLES[table_name], delimiter=",", encoding="windows-1251")
        engine = get_engine()
...
def load_to_dm(conn, table_name):
    try:
        synced_rows = execute_sql_file(conn, f"sql/dm/dml/sync_{table_name}.sql")
        conn.commit()
...
def main():
    with get_connection() as conn:
        for table_name in DM_TABLES:
            create_stage_schema(conn)
            stage_rows = load_to_stage(table_name)
            synced_rows = load_to_dm(conn, table_name)
